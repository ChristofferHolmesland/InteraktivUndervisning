			...
			<b-col cols="12" lg="8" class="mb-3">
				<b-row class="mb-3">
					<b-col cols="4">
						<b-button
							block
							@click="returnToClientDashboard"
							variant="primary"
							>{{ getLocale.backBtn }}</b-button
						>
					</b-col>
					<b-col>
						<b-form-select
							...
							<b-row>
								<b-col>
									<b-form-input
										type="text"
										:placeholder="
											getLocale.textInputPlaceholder
										"
									/>
								</b-col>
							</b-row>
						...
						<div v-if="questionType === 'Shellsort'">
							...
							>
								<b-row class="shellsortTableRow">
									...
									<div
										v-for="(value,
										index) in shellsortStartList"
										:key="index"
										class="shellsortColArrayElement"
									>
										{{ index }}
									</div>
								</b-row>
								<b-row class="mb-2 shellsortTableRow">
									<div class="shellsortColKValue">
										<input
											type="text"
											disabled
											:value="shellsortStartKValue"
											class="shellsortInput"
										/>
									</div>
									<div
										v-for="(value,
								...
								<b-row
									v-for="row in shellsortRows"
									:key="row"
									class="mb-1 shellsortTableRow"
								>
									<div class="shellsortColKValue">
										<input
											type="text"
											placeholder="Enter k value"
											v-model="shellsortKValues[row - 1]"
											class="shellsortInput"
										/>
									</div>
									<div
										v-for="(value,
										index) in getShellsortRowValues(
											row - 1
										)"
										:key="index"
										@click="
											shellsortElementClicked(
												row - 1,
												index
											)
										"
							...
							<b-container>
								<b-row>
									<b-col cols="6" style="text-align: center;">
										<b-btn
											variant="primary"
											@click="shellsortAddNewLine"
											>{{ getLocale.newRowBtn }}</b-btn
										>
									</b-col>
									<b-col cols="6" style="text-align: center;">
										<b-btn
											variant="danger"
											@click="shellsortRemoveLastLine"
											>{{ getLocale.removeRowBtn }}</b-btn
										>
									</b-col>
								</b-row>
							</b-container>
				...
				<b-row v-if="getShowSettings" id="SandBoxSettings">
					<b-col>
						...
						<div v-show="showSettings">
							<b-container
								class="jumbotron"
								v-if="questionType === 'Shellsort'"
							>
								<b-row>
									<b-col>
										<b-form-group>
											<label for="shellsortStartKValue">{{
												getSettingObject.kValueLabel
											}}</label>
											<b-form-input
												id="shellsortStartKValue"
												v-model="shellsortStartKValue"
												@change="
													shellsortInitialValuesChange
												"
											>
											</b-form-input>
											<label for="shellsortStartList">{{
												getSettingObject.listLabel
											}}</label>
											<b-form-input
												id="shellsortStartList"
												v-model="shellsortStartString"
												@change="
													shellsortInitialValuesChange
												"
											>
							...
							<b-container
								class="jumbotron"
								v-if="questionType === 'Quicksort'"
							>
								<b-row>
									<b-col>
										<b-form-group>
											<label for="quicksortStartList">{{
												getSettingObject.listLabel
											}}</label>
											<b-form-input
												id="quicksortStartList"
												v-model="quicksortString"
												@change="
													quicksortInitialValuesChange
												"
											>
											</b-form-input>
										</b-form-group>
							...
							<b-container
								class="jumbotron"
								v-if="questionType === 'Mergesort'"
							>
								<b-row>
									<b-col>
										<b-form-group>
											<label for="mergesortStartList">{{
												getSettingObject.listLabel
											}}</label>
											<b-form-input
												id="mergesortStartList"
												v-model="mergesortString"
												@change="
													mergesortInitialValuesChange
												"
											>
											</b-form-input>
										</b-form-group>
							...
							<b-container
								class="jumbotron"
								v-if="questionType === 'Python'"
							>
								<b-row>
									<b-col>
										<b-button
											variant="primary"
											@click="parsePythonCode"
											>{{
												getSettingObject.buttonLabel
											}}</b-button
										>
										<b-form-textarea
											id="pythonCodeInput"
											:placeholder="
												getSettingObject.textareaPlaceholder
											"
											ref="codeInput"
											v-model="pythonCode"
											@keydown.native.tab="
...
export default {
	...
	methods: {
		...
		changeSelect: function() {
			...
			if (
				this.questionType === "Mergesort" ||
				this.questionType === "Quicksort" ||
		...
		selectChoice: function(index) {
			this.multipleChoiceChoices[index].selected = !this
				.multipleChoiceChoices[index].selected;
		},
		shellsortAddNewLine() {
			let tempList = [];
			if (this.shellsortRows > 0)
				tempList = this.shellsortRowValues[this.shellsortRows - 1];
