		...
		<b-row class="center middleRow row align-item-center">
			<div v-if="showError">
				...
				>
					...
					<b-button style="width: 150px;" @click="refresh">{{
						getLocale.refreshBtn
					}}</b-button>
				</b-alert>
			</div>
			<div v-else>
		...
		<b-row class="center lastRow row">
			<b-col cols="12">
				<b-button
					...
					class="btn"
					:disabled="
						lastSessionBasicInfo === undefined ? true : false
					"
				>
					{{ getLocale.goBtn }}
				</b-button>
...
export default {
	...
	methods: {
		goToLastSession() {
			...
			if (isNaN(this.lastSessionBasicInfo.id)) return;
			this.$socket.emit(
				"getSessionInformationRequest",
				this.lastSessionBasicInfo.id
			);
