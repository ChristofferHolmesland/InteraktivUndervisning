...
export default class Sort {
	...
	*/
	_findArrayFromNodeValues(values) {
		for (let ai = 0; ai < this.arrays.length; ai++) {
			let nodes = this.arrays[ai].nodes;
			if (nodes.length !== values.length) continue;

			let count = 0;
			for (let i = 0; i < values.length; i++) {
				let found = false;
				for (let ni = 0; ni < nodes.length; ni++) {
					if (values[i] == nodes[ni].v) {
						found = true;
						count++;
						break;
					}
				...
				// If value isn't found in the array, the array
				// can't match all the value
				if (!found) {
					break;
				}
			}
