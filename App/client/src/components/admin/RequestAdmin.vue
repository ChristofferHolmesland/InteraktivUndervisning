				...
				<b-card no-body align-h="center">
					<b-tabs v-model="tabIndex" card>
						<b-tab :title="getLocale.administrator">
							<b-container>
								...
								<b-row>
									<b-col cols="8">
										<b-form-select
											:options="getNoUserRightCoursList"
											v-model="selectedCourse"
										>
											<template
												slot="first"
												v-if="
													getNoUserRightCoursListLength ===
														0
												"
											>
												<option
						...
						<b-tab :title="getLocale.adminApplications">
							<b-container>
								<b-row
									v-for="course in getAppliedList"
									:key="course.id"
								>
									<b-col cols="8">
										{{
...
export default {
	...
	computed: {
		...
		getNoUserRightCoursList: function() {
			...
			for (let i = 0; i < this.noUserRightCoursList.length; i++) {
				...
				response.push({
					text: `${course.code} ${
						this.getLocale.season[course.season]
					} ${course.year}`,
					value: course.id
				});
			}
	...
	methods: {
		...
		applyUserRight: function() {
			...
			this.$socket.emit(
				"applyUserRightRequest",
				this.selectedCourse,
				this.selecetedUserRight
			);
		},
	...
	sockets: {
		requestAdminInfoResponse: function(data) {
			...
			if (this.noUserRightCoursList.length > 0)
				this.selectedCourse = this.noUserRightCoursList[0].id;
