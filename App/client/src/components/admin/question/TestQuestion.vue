<template>
	<div>
		...
		>
			<b-container>
				<b-row @click="changeShowSettings" style="cursor: pointer;">
					<b-col cols="10">
				...
				<b-row v-if="showResult" class="mt-3">
					<b-col cols="12">
						<b-alert
							show
							dismissible
						...
						>
							{{
								result === 1
									? getLocale.resultCorrect
									: getLocale.resultIncorrect
							}}
						</b-alert>
					</b-col>
				</b-row>
					...
					<b-col>
						<b-card no-body>
							<b-tabs
								@input="reloads"
								card
								v-model="mainTabIndex"
							>
								<b-tab
									:title="getLocale.mainTabQuestion"
								...
								<b-tab
									:title="getLocale.mainTabSolution"
									title-item-class="w-50"
								>
									<b-container class="px-0">
										<b-row>
...
export default {
	...
	computed: {
		...
		getTimeLeft: function() {
			...
			return `${this.getLocale.timeLeftLabelText}: ${min.padStart(
				2,
				"0"
			)}:${sec.padStart(2, "0")}`;
		},
		...
		getExtraDesc: function() {
			let order = [];
			let extraDescLocales = this.question.object.questionTypeDesc.locale;
			let extraDescText = this.question.object.questionTypeDesc.text;
		...
		getQuestionTypeName: function() {
			...
			return this.$store.getters.getQuestionTypes[this.question.type - 1].name;
		},
		...
		getImgSrc() {
			let file = this.question.object.files[this.selectedImageIndex];
		...
		getTableColumn: function() {
			return this.question.object.tables[this.selectedTableIndex][0]
				.length;
		},
		getTable: function() {
