			...
			<b-col>
				<b-card no-body>
					<b-tabs card @input="setTab($event)">
						...
						<b-tab :title="getLocale.solution">
							<div v-if="tabIndex === 1">
								...
								<MultipleChoiceSolution
									v-if="
										resultInfo.question.type === 2 &&
											!reload
									"
									:solutions="resultInfo.solution"
									:choices="
										resultInfo.question.object
											.multipleChoices
									"
								/>
								<ShellsortSolution
									v-if="
										resultInfo.question.type === 3 &&
											!reload
									"
									:solution="resultInfo.solution"
								/>
								...
								<TreeSolution
									v-if="
										(resultInfo.question.type === 7 ||
											resultInfo.question.type === 8) &&
											!reload
									"
									:solution="resultInfo.solution"
									ref="graphdrawerContainer"
						...
						<b-tab
							:title="getLocale.answer"
							v-if="resultInfo.answerList.length > 0"
						>
							<div v-if="tabIndex === 2">
					...
					<b-button
						variant="success"
						@click="markAsCorrect"
						v-if="tabIndex === 2 && admin"
						>{{ getLocale.markAsCorrectBtn }}</b-button
					>
				</b-card>
			</b-col>
		</b-row>
...
export default {
	...
	computed: {
		...
		getExtraDesc: function() {
			...
			let extraDescText = this.resultInfo.question.object.questionTypeDesc
				.text;
		...
		showMedia: function() {
			if (this.getImagesLength > 0 || this.getTablesLength > 0) {
				return true;
			}
			return false;
		},
		getTableRow: function() {
			return this.resultInfo.question.object.tables[
