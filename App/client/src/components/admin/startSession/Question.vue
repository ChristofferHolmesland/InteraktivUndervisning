	...
	<b-container>
		...
		<b-row class="text-center" align-h="around">
			<b-col cols="2" v-if="timeLeft > 0">
				<b v-if="timeLeft !== undefined" style="line-height: 100%"
					>{{ getLocale.timeLeft }} {{ getTimeUpdate }}</b
				>
			</b-col>
			<b-col cols="2">
			...
			<b-col cols="2">
				<b-button @click="btnNextClick" variant="primary">{{
					getLocale.nextquestionBtn
				}}</b-button>
			</b-col>
		</b-row>
		<b-row>
				...
				<b-col>
					<b-card no-body>
						<b-tabs card @input="setMediaTab($event)">
							<b-tab
								:title="getLocale.tabExtraInfo"
								:disabled="
									getExtraDesc.length > 0 ? false : true
								"
							>
								<div
									class="displayInline"
									v-for="(info, index) in getExtraDesc"
									:key="index"
								>
							...
							<b-tab
								:title="getLocale.tabImage"
								:disabled="getImagesLength > 0 ? false : true"
							>
							...
							<b-tab
								:title="getLocale.tabTable"
								:disabled="getTablesLength > 0 ? false : true"
							>
								<b-container v-if="mediaTab === 2">
									...
									<b-row
										style="text-align: center;"
										v-if="getTablesLength > 1"
									>
										<b-col cols="4">
											<b-button
												variant="primary"
												@click="changeSelectedTable(-1)"
												>{{
													getLocale.previousBtn
												}}</b-button
											>
